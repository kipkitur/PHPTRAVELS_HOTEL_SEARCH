...
class PhpTravelsHomePageAlzer:
    ...
    ALZER = (By.XPATH, "//div[.='Alzer Hotel Istanbul, Istanbul']")
    ...
    # 2 adults 0 children
    def enter_alzer_details(self, hotel_name, checkin_date, checkout_date):
        destination_input_field = self.browser.find_element(*self.DESTINATION_INPUT)
        destination_input_field.click()
        hotel = self.browser.find_element(*self.ALZER)
        hotel.click()
    ...
    # 2 adults 2 children
    def enter_alzer_details_children_increase(self, hotel_name, checkin_date, checkout_date):
        destination_input_field = self.browser.find_element(*self.DESTINATION_INPUT)
        destination_input_field.click()
        hotel = self.browser.find_element(*self.ALZER)
        hotel.click()
    ...
    # 0 adults 0 children
    def enter_alzer_details_adult_decrease(self, hotel_name, checkin_date, checkout_date):
        destination_input_field = self.browser.find_element(*self.DESTINATION_INPUT)
        destination_input_field.click()
        hotel = self.browser.find_element(*self.ALZER)
        hotel.click()
